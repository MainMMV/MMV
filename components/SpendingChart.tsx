  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 border border-gray-200 dark:border-gray-700 shadow-sm mb-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="font-bold text-gray-900 dark:text-white">Current Month Spending</h3>
        ...
        {chartData.map((d, i) => {
          ...
          return (
            <div key={d.category} className="flex-1 flex flex-col items-center group relative h-full justify-end">
              {/* Tooltip */}
              <div className="absolute bottom-full mb-2 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900 text-white text-xs py-1.5 px-2.5 rounded-lg pointer-events-none whitespace-nowrap z-20 shadow-xl font-medium transform translate-y-2 group-hover:translate-y-0 transition-transform">
                {d.category}: ${d.amount.toLocaleString()}
                <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-gray-900"></div>
              </div>
              
              {/* Value label on top of bar for easy reading on larger screens */}
              <span className="hidden sm:block text-[10px] font-bold text-gray-400 mb-1 transition-all group-hover:text-gray-600 dark:group-hover:text-gray-300 group-hover:-translate-y-1">
              ...
              <div className="h-8 flex items-start justify-center w-full mt-2">
                 <span className="text-[10px] sm:text-xs font-medium text-gray-500 dark:text-gray-400 text-center leading-tight line-clamp-2 w-full break-words">
                    {d.category}
                 </span>
              </div>
            </div>
