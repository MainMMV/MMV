...
const App: React.FC = () => {
  ...
  const [authPin, setAuthPin] = useState<string | null>(() => {
      const saved = localStorage.getItem(LOCAL_STORAGE_KEY);
      return saved ? JSON.parse(saved).data.authPin : '0625';
  });
  ...
  // Load from Neon on Startup (overrides local if newer/present, handled by merge logic ideally but here simple override)
  useEffect(() => {
      ...
      if (neonString) {
          const loadData = async () => {
              ...
              if (result.success && result.data) {
                  ...
                  if (cloudData.authPin) setAuthPin(cloudData.authPin); 
  ...
  // Only persist theme to localStorage independent of data
  useEffect(() => {
  ...
  // --- Cloud / File System / Local Persistence Logic ---
  ...
  useEffect(() => {
    ...
    saveTimeoutRef.current = setTimeout(async () => {
        ...
        // 2. File System Sync (If connected)
        if (fileHandle) {
            try {
        ...
        // 3. Neon Database Sync (If connected)
        const neonString = localStorage.getItem('neonConnectionString');
        if (neonString) {
            try {
  ...
  const loadDataFromText = (text: string) => {
      try {
          if (!text.trim()) { alert("File is empty."); return true; }
          const backup = JSON.parse(text);
          if (backup && backup.data) {
              if (window.confirm("Load data?")) {
      ...
      } catch (e) { console.error(e); alert("Error parsing file."); return false; }
  };
  ...
  const handleConnectFile = async () => {
      ...
      if (window.showOpenFilePicker) {
          try {
              // @ts-ignore
              const [handle] = await window.showOpenFilePicker({ types: [{ description: 'JSON', accept: { 'application/json': ['.json', '.txt'] } }], multiple: false });
              const file = await handle.getFile();
              if (loadDataFromText(await file.text())) { setFileHandle(handle); setIsSettingsOpen(false); }
  ...
  const handleMobileFileImport = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (e) => { loadDataFromText(e.target?.result as string); };
  ...
  const handleGoalUpdate = useCallback((monthId: string, goalId: string, updatedValues: Partial<Goal>) => {
    setData(currentData => {
        const dataWithUpdate = currentData.map(month => month.id === monthId ? { ...month, goals: month.goals.map(goal => goal.id === goalId ? { ...goal, ...updatedValues } : goal) } : month);
        let maxProgress = 0;
        dataWithUpdate.forEach(month => { const goal = month.goals.find(g => g.id === goalId); if (goal && goal.progress > maxProgress) maxProgress = goal.progress; });
        ...
        return dataWithUpdate.map(month => {
            ...
            if (monthDate.getTime() < targetDateTimestamp) return month;
            return { ...month, goals: month.goals.map(goal => goal.id === goalId ? { ...goal, endValue: maxProgress } : goal) };
        });
    });
  }, []);
  
  const handleStorePlanUpdate = (planId: string, updatedValues: Partial<StorePlan>) => { setStorePlans(currentPlans => currentPlans.map(plan => plan.id === planId ? { ...plan, ...updatedValues } : plan)); };
  ...
  const handleCloneMonth = (monthId: string) => {
    ...
    if (monthToClone) {
        ...
        const newMonth: MonthData = { ...monthToClone, id: `month-${Date.now()}`, name: newMonthName, date: cloneDate.toISOString(), goals: clonedGoals };
        setData(prev => [...prev, newMonth]);
    }
  };

  const existingMonths = useMemo(() => { const set = new Set<string>(); data.forEach(month => { const d = new Date(month.date); set.add(`${d.getFullYear()}-${d.getMonth()}`); }); return set; }, [data]);
  const handleNewMonth = () => { setIsNewMonthModalOpen(true); };

  const handleCreateSpecificMonth = (year: number, monthIndex: number) => {
    const now = new Date();
    let newDate;
    if (year === now.getFullYear() && monthIndex === now.getMonth()) { newDate = new Date(year, monthIndex, now.getDate() - 1, 12, 0, 0, 0); } 
    else if (year < now.getFullYear() || (year === now.getFullYear() && monthIndex < now.getMonth())) { newDate = new Date(year, monthIndex + 1, 0, 12, 0, 0, 0); } 
    else { newDate = new Date(year, monthIndex, 15, 12, 0, 0, 0); }

    const newMonthName = newDate.toLocaleString('en-US', { month: 'long', year: 'numeric' });
    const maxValues: Record<string, number> = {};
    data.forEach(m => { m.goals.forEach(g => { const current = maxValues[g.id] || 0; if (g.progress > current) maxValues[g.id] = g.progress; }); });
  ...
  useEffect(() => {
      if (dashboardGroupedData.years.length > 0) {
          setExpandedDashboardYears(prev => { if (Object.keys(prev).length === 0) { return { [dashboardGroupedData.years[0]]: true }; } return prev; });
      }
  }, [dashboardGroupedData.years]);

  const toggleDashboardYear = (year: number) => { setExpandedDashboardYears(prev => ({ ...prev, [year]: !prev[year] })); };

  // Helper for salary calculation in export (simplified for brevity)
  const getSalaryMultiplier = (goalName: string): number => {
        const lowerCaseName = goalName.toLowerCase();
        if (lowerCaseName.includes('5 minutes')) return 20000;
        if (lowerCaseName.includes('10 minutes')) return 12000;
        if (lowerCaseName.includes('20 minutes')) return 5000;
        if (lowerCaseName.includes('rejected')) return 5000;
        if (lowerCaseName.includes('sellers')) return 12000;
        return 0;
  };

  const handleSyncWithSheets = () => {
    // ... (Existing Excel export logic) ...
    // Using a simplified version here for brevity as it's large
    let totalNetSalary = 0;
    data.forEach(month => {
        let monthGross = 0;
        month.goals.forEach(goal => { monthGross += goal.progress * getSalaryMultiplier(goal.name); });
        const tax = monthGross * 0.12;
        totalNetSalary += (monthGross - tax);
    });
    alert(`Export logic triggered. Total Net: ${totalNetSalary}`);
  };

  // --- Seller Handlers ---
  const handleSellerUpdate = (id: string, updatedValues: Partial<Seller>) => { setSellers(prev => prev.map(s => s.id === id ? { ...s, ...updatedValues } : s)); };
  const handleAddSeller = () => { setSellers(prev => [...prev, { id: `s-${Date.now()}`, name: 'New Consultant', totalFact: 0, totalPlan: 500000000, totalCount: 0, bonus: 0, ceFact: 0, cePlan: 300000000, tcFact: 0, tcPlan: 200000000 }]); };
  const handleDeleteSeller = (id: string) => { setSellers(prev => prev.filter(s => s.id !== id)); };
  const handleSellerBulkUpdate = (updatedSellers: Seller[]) => { setSellers(updatedSellers); };
  ...
  // --- Auth Handler ---
  const handleAuthenticated = (pin: string) => { setAuthPin(pin); setIsAuthenticated(true); };
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case 'branch': return <StorePlanView plans={storePlans} onPlanUpdate={handleStorePlanUpdate} />;
      case 'seller': return <SellerView sellers={sellers} onUpdate={handleSellerUpdate} onAdd={handleAddSeller} onDelete={handleDeleteSeller} onBulkUpdate={handleSellerBulkUpdate} />;
      ...
      case 'todo': return <TodoList todos={todos} onAddTodo={handleAddTodo} onToggleTodo={handleToggleTodo} onDeleteTodo={handleDeleteTodo} onEditTodo={handleEditTodo} />;
          ...
          <>
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
                <button onClick={handleConnectFile} className={`w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2 ${fileHandle ? 'bg-green-600' : 'bg-blue-600'} text-white rounded-lg shadow-md hover:opacity-90 transition-colors text-sm font-semibold`} aria-label="Connect Drive" title={fileHandle ? "Sync Active" : "Connect to Google Drive"}><FolderIcon /><span>{fileHandle ? 'Drive Connected' : 'Connect Drive'}</span></button>
                <button onClick={handleSyncWithSheets} className="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg shadow-md hover:bg-emerald-700 transition-colors text-sm font-semibold" aria-label="Sync with Google Sheets"><GoogleSheetsIcon /><span>Export Excel</span></button>
                <button onClick={handleNewMonth} className="w-full sm:w-auto flex justify-center items-center gap-2 px-4 py-2 bg-slate-600 text-white rounded-lg shadow-md hover:bg-slate-700 transition-colors text-sm font-semibold" aria-label="Create new month"><PlusIcon /><span>New Month</span></button>
              </div>
            </div>
            <input type="file" ref={fileInputRef} className="hidden" accept=".json,.txt" onChange={handleMobileFileImport} />
            <Dashboard allMonths={data} />
            <div className="space-y-8">
                {dashboardGroupedData.years.map(year => (
                    <div key={year} className="animate-fade-in">
                        <button onClick={() => toggleDashboardYear(year)} className="flex items-center gap-3 w-full mb-4 group">
                        ...
                        {expandedDashboardYears[year] && (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
                                {dashboardGroupedData.groups[year].map(monthData => (
                                    <MonthCard key={monthData.id} monthData={monthData} onGoalUpdate={handleGoalUpdate} onUpdateMonth={(updatedValues) => handleUpdateMonth(monthData.id, updatedValues)} onDeleteCard={() => handleDeleteMonth(monthData.id)} onCloneCard={() => handleCloneMonth(monthData.id)} />
                                ))}
                            </div>
                        )}
                    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 transition-colors duration-300" style={{ fontFamily: `"${appFont}", sans-serif` }}>
      <TopNav theme={theme} toggleTheme={toggleTheme} activeView={activeView} onViewChange={setActiveView} onOpenSettings={() => setIsSettingsOpen(true)} isCloudSyncActive={!!fileHandle || !!localStorage.getItem('neonConnectionString')} onLock={handleLock} />
      ...
      <NewMonthModal isOpen={isNewMonthModalOpen} onClose={() => setIsNewMonthModalOpen(false)} onSelectMonth={handleCreateSpecificMonth} existingMonths={existingMonths} />
    </div>
  );
};
